"use strict";
// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
Object.defineProperty(exports, "__esModule", { value: true });
exports.getServiceTemplateOutput = exports.getServiceTemplate = void 0;
const pulumi = require("@pulumi/pulumi");
const utilities = require("../utilities");
/**
 * Definition of AWS::Proton::ServiceTemplate Resource Type
 */
function getServiceTemplate(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("aws-native:proton:getServiceTemplate", {
        "arn": args.arn,
    }, opts);
}
exports.getServiceTemplate = getServiceTemplate;
/**
 * Definition of AWS::Proton::ServiceTemplate Resource Type
 */
function getServiceTemplateOutput(args, opts) {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("aws-native:proton:getServiceTemplate", {
        "arn": args.arn,
    }, opts);
}
exports.getServiceTemplateOutput = getServiceTemplateOutput;
//# sourceMappingURL=getServiceTemplate.js.map